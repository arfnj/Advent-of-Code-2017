...
// banks get two blocks each, and the third bank gets one back. The final result looks like this: 2
// 4 1 2.
...
(() => {
	...
	while (!found) {
		...
		temp[index] = 0;
		for (let i = 1; i <= max; i++) {
			temp[(index+i)%temp.length] ++;
		}
		count ++;
		const current = temp.join(',');
		if (seen.has(current)) {
			found = true;
...
(() => {
	...
	while (!found) {
		...
		if (seen.hasOwnProperty(current)) {
			found = true;
			cycle = count - seen[current];
		} else seen[current] = count;
	}
	console.log(cycle);
})();
