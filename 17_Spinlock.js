...
// input) before inserting the first new value, 1, after the value it stopped on. The inserted value
// becomes the current position. Then, it steps forward from there the same number of steps, and
// wherever it stops, inserts after it the second new value, 2, and uses that as the new current
// position again.
...
const takeASpin = (spinlock,steps,rounds) => {
	let spinCopy = spinlock.slice()
	let position = 0;
	for (let i = 1; i <= rounds; i ++) {
		let len = spinCopy.length;
		let spot = (position+steps)%len;
		spinCopy = spot !== len-1 ? [...spinCopy.slice(0,spot+1),i,...spinCopy.slice(spot+1)] : [...spinCopy.slice(0,spot+1),i];
		position = spot + 1;
	}
	console.log(spinCopy[position+1]);
}

takeASpin(spinlock,steps,2017);
...
const fasterSpin = (spinlock,steps,rounds) => {
	let len = spinlock.length
	let position = 0;
	let nextDoor = null;
	for (let i = 1; i <= rounds; i ++) {
		let spot = (position+steps)%len;
		nextDoor = spot === 0 ? i : nextDoor
		len++;
		position = spot + 1;
	}
...
fasterSpin(spinlock,steps,50000000);
