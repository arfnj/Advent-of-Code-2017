...
const input = `157	564	120	495	194	520	510	618	244	443	471	473	612	149	506	138
...
input
  .split('\n')
  .map(row => row.split('\t'))
  .reduce((checksum, row) => checksum + Math.max(...row) - Math.min(...row), 0);
...
input
  ...
  .map(row => row.split('\t'))
  .reduce((checksum, row) => {
  	let sorted = row.map(val => Number(val)).sort((a,b) => a-b);
  	finder:
			for (let i=0; i<sorted.length; i++) {
				for (let j=i+1; j<sorted.length; j++) {
					if (sorted[j]%sorted[i] === 0) {
						checksum += (sorted[j]/sorted[i]);
						found = true;
						break finder;
					}
				}
