...
const partOne = list => {
	...
	phrases.forEach(phrase => {
		const temp = new Set(phrase);
		if (temp.size === phrase.length) {
			valid ++;
		}
	})
...
const anagramChecker = (setA, setB) => {
...
const partTwo = list => {
	const phrases = list.split('\n').map(phrase => phrase.split(' '));
	let valid = phrases.reduce((count,phrase)  => {
		let hasAnagrams = false;
		const sets = phrase.map(word => new Set(word));
		loop:
			for (let i=0; i<sets.length; i++) {
				for (let j=i+1; j<sets.length; j++) {
					if (anagramChecker(sets[i],sets[j])) {
						hasAnagrams = true;
						break loop;
					}
				}
			}
		return hasAnagrams ? count : count + 1;
	},0)	
