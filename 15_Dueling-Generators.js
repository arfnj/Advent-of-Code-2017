...
const generators = (aStart, bStart) => {
	let matches = 0;
	let a = aStart;
	let b = bStart;
	for (let i=1; i<=40000000; i++) {
		a = (a*16807)%2147483647;
		aBin = a.toString(2).slice(-16);
		b = (b*48271)%2147483647;
		bBin = b.toString(2).slice(-16);
		matches = aBin === bBin ? matches +1 : matches;
	}
	console.log(`${matches} matches for part 1`);
}
...
const part2 = (aStart,bStart) => {
	let matches = 0;
	let a = aStart;
	let b = bStart;
	let aBin,bBin;
	
	const generator = (val,letter) => {
		const multiplier = letter === 'a' ? 16807 : 48271;
		return (val*multiplier)%2147483647;
	}
	...
	console.log(`${matches} matches for part 2`);
}
