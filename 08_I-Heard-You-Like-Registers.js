...
let instructions = input.split('\n').map(line => line.split(' ')).map(line => {
	let arr = [];
	arr.push(line[0]);
	arr[1] = line[1] === 'inc' ? Number(line[2]) : -1*line[2];
	arr.push(line[4],line[5],Number(line[6]));
	return arr;
});
...
(() => {
	...
	const isTrue = (left, operator, right) => {
		if (!registers.hasOwnProperty(left)) {
			registers[left] = 0;
		}
		return operators[operator](registers[left],right);
	}
	
	instructions.forEach(instruction => {
		if (!registers.hasOwnProperty(instruction[0])) {
			registers[instruction[0]] = 0;
		}
		registers[instruction[0]] = isTrue(instruction[2],instruction[3],instruction[4]) ? registers[instruction[0]] + instruction[1] : registers[instruction[0]];
		max = Math.max(max,registers[instruction[0]]);
	})
	console.log('Max final value:',Math.max(...Object.values(registers)));
	console.log('Max anytime value:',max);
})()
