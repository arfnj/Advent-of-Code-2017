...
(() => {
	...
	// 	 distance += matrix[key[pathArr[i]]][key[pathArr[i-1]]];
	// }
	// console.log(distance);
	// pathArr.forEach(dir => key[dir]++);
	pathArr.forEach(dir => key[dir]());
	directions[0] = axes.ns >= 0 ? 'n' : 's';
	directions[1] = axes.nesw >= 0 ? 'ne' : 'sw';
	directions[2] = axes.nwse >= 0 ? 'nw' : 'se';
	console.log(axes,directions);
	let ring = Math.abs(axes.ns);
	console.log('NS ring:', ring);
	if (directions[0] === 'n') {
		ring = directions[1] === 'ne' ? ring + axes.nesw : Math.max(Math.abs(axes.nesw), ring);
	} else ring = directions[1] === 'se' ? ring + Math.abs(axes.nesw) : Math.max(axes.nesw, ring);
	console.log('NESW ring:', ring);		
	if (directions[1] === 'ne') {
			if (directions[2] === 'nw') {
				ring += axes.nwse;
			} else {
				const steps = Math.abs(axes.nwse);
				if (steps > 0 && steps <= ring) {
					ring --;
				} else if (steps >= (ring+1)) {
					ring = steps - 1;
				}
			}	
	} else {
		if (directions[2] === 'se') {
			ring += Math.abs(axes.nwse);
		} else {
			if (axes.nwse > 0 && axes.nwse <= ring) {
				ring --;
			} else if (axes.nwse >= (ring+1)) {
				ring = axes.nwse - 1;
			}
		}
	}
