...
// disc, are other programs, each holding their own disc, and so on. At the very tops of these sub-
...
cleaned = cleaned.map(program => program.split(' -> '));
cleaned = cleaned.map(program => {
  let arr = [];
  let chopped = program[0].split(' (');
  arr.push(chopped[0],Number(chopped[1].slice(0,-1)));
  if (program[1]) {
    arr.push(program[1].split(', '));
  }
  return arr;
});
let inputObj = {};
cleaned.forEach(program => inputObj[program[0]] = {
  weight: program[1],
  supporting: program[2]
});

/* inputObj.lgxjcy = { weight: 359, supporting: [ vsbfmt, kbdcl ] } */

const findBottom = () => {
  let supported = new Set(Object.keys(inputObj));
  for (var program in inputObj) {
    if (inputObj[program].supporting) {
      inputObj[program].supporting.forEach(other => {
        if (supported.has(other)) {
          supported.delete(other);
        }
      });
    }
  }
  console.log(supported.entries().next().value[0]);
  return supported.entries().next().value[0];
};
...
const weigher = node => {
  if (!node.supporting) {
    ...
    node.supporting.forEach(supported => total += weigher(inputObj[supported]));
    node.totalWeight = total;
  }
  return node.totalWeight;
}

weigher(inputObj[bottom]);

const oddManOut = (node, diff) => {
  if (node.supporting) {
    let weights = [];
    let found = false;
    let odd;
    let others;
    node.supporting.forEach(supported => weights.push(inputObj[supported].totalWeight));
    let values = new Set(weights);
    for (let value of values) {
      if (weights.indexOf(value) === weights.lastIndexOf(value)) {
        odd = value;
        found = true;
        others = (weights.reduce((a,b) => a+b,0) - odd) / (weights.length-1);
        break;
      }
    }
    if (found) {
      oddManOut(inputObj[node.supporting[weights.indexOf(odd)]], odd-others);
    } else { 
